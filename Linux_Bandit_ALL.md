...
According to https://geek-university.com/specify-port-ranges/, nmap allows scnning through a range of ports: 

```
nmap -p [range_start]-[range_end] [ip]
```

this returns:

```
Starting Nmap 7.40 ( https://nmap.org ) at 2022-08-01 18:25 CEST
...
After trying 
```
echo cluFn7wTiGryunymYOu4RcffSxQluehd | openssl s_client -ign_eof -connect localhost:[port]
```
I received an RSA key at 31790. Then, I created a file at /tmp and saved the rsa key. 
...
I received an error, informing my permissions were way too broad. I had to do:
...
```
which entered bandit 17 server. There are two files:
...
Each one with some passwords. After noticing none of them works, I remembered `etc/bandit_pass/` holds the password for every user. After doing: 
...
### bandit 15->16 password: xLYVMN9WE5zQ5vHacb0sZEVqbrp7nBTn
-----------------------------------------------------------
# bandit17 -> bandit18
